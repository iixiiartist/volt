...
pub async fn provision_file(manifest: PathBuf, marketplace_verified: bool, settings: &Settings) -> anyhow::Result<()> {
...
pub async fn provision_remote(pkg_id: String, registry_base_url: Option<String>, auth_token: Option<String>, settings: &Settings) -> anyhow::Result<()> {
    let pool = db::connect(&settings.database_url).await?;
