...
use crate::models::*;
use crate::{db, worker, session};
use crate::tui::TuiChat;
use super::AgentMode;

pub async fn run(options: AgentTuiOptions) -> anyhow::Result<()> {
    let AgentTuiOptions { model, allow, max_iterations, mode, settings } = options;
    ...
    let config = AgentConfig {
        ...
        system_prompt: None, max_iterations: max_iterations.unwrap_or(25),
        temperature: 0.3, toolsets: vec!["builtin".into()], hidden: false,
        allow_all: allow, enabled_context_kinds: mode_profile.context_kinds(),
    ...
    tokio::spawn(worker::seed_background(context_store.clone(), embedder.clone(), tools.clone(), settings.sandbox_policy.clone()));
    ...
    if let Ok(pool) = db::connect(&settings.database_url).await {
        ...
        tokio::spawn(async move { worker::seed_skills_from_db(&context_store, &pool, &skill_emb).await; });
    } else { agent = agent.with_memory_embedder_only(embedder); }

    println!("[mode] {} — {} context kinds", mode, mode_profile.context_kinds().len());
    ...
    if let Ok(sp) = session::open_sessions(&std::path::PathBuf::from("volt_sessions.db")).await {
        if let Ok(sessions) = session::list_sessions(&sp, 10).await {
            if !sessions.is_empty() {
                ...
                let answer = tokio::task::spawn_blocking(|| { let mut buf = String::new(); std::io::stdin().read_line(&mut buf).ok(); buf }).await.unwrap_or_default();
                if let Ok(idx) = answer.trim().parse::<usize>() {
                    if let Some(s) = idx.checked_sub(1).and_then(|i| sessions.get(i)) {
                        if let Ok(msgs) = session::load_messages(&sp, s.id).await {
                            ...
                            state.session_id = s.id; state.messages = msgs;
                        }
                    }
                }
            }
...
pub struct AgentTuiOptions {
    pub model: String, pub allow: bool, pub max_iterations: Option<u32>,
    pub mode: String, pub settings: crate::config::Settings,
}
