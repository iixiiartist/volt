...
pub async fn install_skill(name: String, catalog_url: Option<String>, settings: &Settings) -> anyhow::Result<()> {
    let embedder = EmbeddingClient::new_smart().await;
    let pool = db::connect(&settings.database_url).await?;
    let catalog = catalog::fetch_catalog(catalog_url.as_deref()).await?;
...
pub async fn import_skill(path: PathBuf, format: String, name: Option<String>, settings: &Settings) -> anyhow::Result<()> {
    use crate::skills::importer;

    if !path.exists() { eprintln!("File not found: {:?}", path); std::process::exit(1); }

    let content = tokio::fs::read_to_string(&path).await.map_err(|e| {
        eprintln!("Failed to read {:?}: {}", path, e); anyhow::anyhow!("read error")
    })?;
    ...
    println!("✓ Imported from {} as skill '{}' with RAG embedding.", importer::format_label(&source_fmt), skill_name);
