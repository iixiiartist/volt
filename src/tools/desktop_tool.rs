    ...
    pub async fn r#type(text: &str) -> ToolResult {
        ...
        match Enigo::new(&Settings::default()) {
            Ok(mut enigo) => match enigo.text(text) {
                Ok(_) => ToolResult {
                    success: true,
                    output: format!("typed {} chars", text.len()),
                    error: None,
                    duration_ms: started.elapsed().as_millis(),
                },
                Err(e) => ToolResult {
...
class W {{
    ...
    static void Main() {{
        ...
        let _ = std::process::Command::new("csc")
            .arg("-out:find_window.exe")
            .arg(&cs_path)
            .output();
                ...
                Ok(out) => {
                    ...
                    if output.contains("not found") {
                        ToolResult {
                            success: false,
                            output: String::new(),
                            error: Some(format!("window '{0}' not found", title)),
                            duration_ms: started.elapsed().as_millis(),
                        }
                    } else {
                        ToolResult {
                ...
                .output()
            {
                Ok(out) => {
                    ...
                    if output.contains("not found") {
                        ...
                        ToolResult {
                            success: true,
                            output: format!("found window: {0}", output),
                            error: None,
                            duration_ms: started.elapsed().as_millis(),
                        }
                    }
...
#[cfg(feature = "tools-desktop")]
pub async fn desktop_click(x: i32, y: i32) -> ToolResult {
    desktop_impl::click(x, y).await
}
#[cfg(feature = "tools-desktop")]
pub async fn desktop_type(text: &str) -> ToolResult {
    desktop_impl::r#type(text).await
}
#[cfg(feature = "tools-desktop")]
pub async fn desktop_key(key: &str) -> ToolResult {
    desktop_impl::press_key(key).await
}
#[cfg(feature = "tools-desktop")]
pub async fn desktop_find_window(title: &str) -> ToolResult {
    desktop_impl::find_window(title).await
}

#[cfg(not(feature = "tools-desktop"))]
pub async fn desktop_click(_x: i32, _y: i32) -> ToolResult {
    ToolResult {
        success: false,
        output: String::new(),
        error: Some("Desktop tool not compiled".into()),
        duration_ms: 0,
    }
}
#[cfg(not(feature = "tools-desktop"))]
pub async fn desktop_type(_text: &str) -> ToolResult {
    ToolResult {
        success: false,
        output: String::new(),
        error: Some("Desktop tool not compiled".into()),
...
#[cfg(not(feature = "tools-desktop"))]
pub async fn desktop_key(_key: &str) -> ToolResult {
    ToolResult {
        success: false,
        output: String::new(),
        error: Some("Desktop tool not compiled".into()),
...
#[cfg(not(feature = "tools-desktop"))]
pub async fn desktop_find_window(_title: &str) -> ToolResult {
    ToolResult {
        success: false,
        output: String::new(),
        error: Some("Desktop tool not compiled".into()),
